...
import { Check, CircleAlert, Droplets, ExternalLink, LoaderCircle } from 'lucide-react';
import { RiskBadge } from './RiskBadge';
import { dexScreenerUrl, openSeaUrl } from '../lib/externalLinks';
...
export function TokenTable({tokens,onSelect}:{tokens:TokenRecord[];onSelect:(t:TokenRecord)=>void}) {
 return <div className="table-wrap"><table><thead><tr><th>Age</th><th>Type</th><th>Asset</th><th>Contract</th><th>Market cap</th><th>Holders</th><th>Top 5</th><th>LP</th><th>Analysis</th><th>Risk</th><th>Link</th></tr></thead><tbody>{tokens.map(t=><tr key={`${t.chainKey}:${t.address}`} onClick={()=>onSelect(t)}>
  ...
  <td><RiskBadge label={t.riskLabel} score={t.riskScore}/></td><td><a className="market-link" href={t.assetType==='ERC20'?dexScreenerUrl(t.address):openSeaUrl(t.chainKey,t.address)} target="_blank" rel="noreferrer" onClick={event=>event.stopPropagation()}>{t.assetType==='ERC20'?'DEX':'OPEN'}<ExternalLink size={13}/></a></td></tr>)}</tbody></table>{!tokens.length&&<div className="empty">No matching contracts detected yet. Start a token, NFT, or combined scan to populate this table.</div>}</div>;
}
