...
export function TokenTable({tokens,onSelect}:{tokens:TokenRecord[];onSelect:(t:TokenRecord)=>void}) {
 return <div className="table-wrap"><table><thead><tr><th>Age</th><th>Token</th><th>Contract</th><th>Market cap</th><th>Holders</th><th>Top 5</th><th>LP</th><th>Analysis</th><th>Risk</th></tr></thead><tbody>{tokens.map(t=><tr key={t.address} onClick={()=>onSelect(t)}>
  ...
  <td className="mono">{usd(t.marketCapUsd)}</td><td>{t.holderCountEstimate??'—'}</td><td>{t.circulatingTop5Percent!=null?`${t.circulatingTop5Percent.toFixed(1)}%`:'—'}</td><td>{t.poolCreated?<span className="ok"><Droplets size={14}/> YES</span>:<span className="dim">—</span>}</td>
