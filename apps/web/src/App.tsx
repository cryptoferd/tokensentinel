...
import { authMe, authNonce, authVerify, clearSessionToken, deleteScan, fetchChains, fetchScanResults, fetchScans, fetchStats, getSessionToken, logout, saveSessionToken, startHistoryScan, startScanner, stopScanner } from './lib/api';
...
const scanLabel=(scan:ScanSession)=>`${scan.chainName} · ${scan.mode==='live'?`${scan.durationMinutes}m live scan`:`Last ${WINDOWS.find(item=>item.value===scan.lookbackMinutes)?.label??`${scan.lookbackMinutes}m`}`} · ${assetLabel(scan.assetType)}`;
...
export default function App(){
  ...
  const [chains,setChains]=useState<ChainOption[]>([]);const [chainKey,setChainKey]=useState('robinhood');
  ...
  const refresh=useCallback(async()=>{
    if(!address)return;
    try{
      const [nextStats,nextScans,nextChains]=await Promise.all([fetchStats(chainKey),fetchScans(),chains.length?Promise.resolve(chains):fetchChains()]); setStats(nextStats);setScans(nextScans);setChains(nextChains);
      const scanId=selectedScanId||nextScans[0]?.id||''; if(!selectedScanId&&scanId)setSelectedScanId(scanId);
      if(scanId){const result=await fetchScanResults(scanId,filters);setTokens(result.items);setSelected(current=>current?result.items.find(token=>token.chainKey===current.chainKey&&token.address===current.address)??current:null);}else setTokens([]);
      setError('');
  ...
  },[address,selectedScanId,chainKey,chains.length,JSON.stringify(filters)]);
  ...
  const selectedChain=chains.find(chain=>chain.key===chainKey)??null;
  ...
  const startLive=()=>action(async()=>{const {scan}=await startScanner(duration,assetType,chainKey);setSelectedScanId(scan.id);});
  const stopLive=()=>action(async()=>{await stopScanner(activeScan?.id);});
  const history=(minutes:number)=>action(async()=>{const {scan}=await startHistoryScan(minutes,assetType,chainKey);setSelectedScanId(scan.id);});
  ...
  return <main>
    <header><div className="brand"><div className="mark"><ShieldCheck/></div><div><h1>MULTICHAIN TOKEN SENTINEL</h1><p>Croikey-gated launch intelligence</p></div></div><div className="header-actions"><div className={`live ${activeScan?'on':''}`}><Wifi size={15}/>{activeScan?'SCANNING':'READY'} <span>{activeScan?.chainName??selectedChain?.shortName??'EVM'}</span></div><button className="wallet-pill" onClick={signOut} title="Sign out"><Wallet size={14}/>{short(address)}<LogOut size={13}/></button></div></header>
    <div className="content"><section className="hero"><div><div className="eyebrow">PRIVATE HOLDER DASHBOARD</div><h2>Your scans.<br/><span>Your launch history.</span></h2><p>Run live monitoring or backfill recent Robinhood Chain launches. Every scan and its results are saved privately to your connected wallet.</p></div><div className="block-card"><span>SCANNED BLOCK</span><strong>{stats?.scannedBlock?.toLocaleString()||'—'}</strong><small>tip {stats?.latestBlock?.toLocaleString()||'—'}</small></div></section>
      <section className="chain-mode"><div><span>SCAN NETWORK</span><p>Each scan runs against one EVM chain and keeps its results separate.</p></div><label><select value={chainKey} disabled={busy||Boolean(activeScan)} onChange={event=>setChainKey(event.target.value)}>{chains.map(chain=><option key={chain.key} value={chain.key} disabled={!chain.enabled}>{chain.name}{chain.enabled?'':' — Alchemy key required'}</option>)}</select><small>CHAIN ID {selectedChain?.id??'—'}</small></label></section>
      ...
      <section className="workspace"><aside className="scan-history"><div className="section-heading"><Clock3 size={15}/><span>SCAN HISTORY</span></div>{scans.map(scan=><div className={`scan-history-item ${scan.id===selectedScanId?'selected':''}`} key={scan.id}><button className="scan-select" onClick={()=>setSelectedScanId(scan.id)} title={scan.error??undefined}><span>{scanLabel(scan)}</span><small>{new Date(scan.startedAt).toLocaleString()}</small><em className={`status-${scan.status}`}>{scan.status}{scan.status==='running'&&scan.totalBlocks?` ${Math.round(scan.scannedBlocks/scan.totalBlocks*100)}%`:''} · {scan.resultCount}</em>{scan.error&&<small className="scan-error-detail">{scan.error}</small>}</button>{scan.status!=='running'&&<button className="delete-scan" disabled={busy} onClick={()=>removeScan(scan)} aria-label={`Delete ${scanLabel(scan)}`} title="Delete scan"><Trash2 size={14}/></button>}</div>)}{!scans.length&&<p>No saved scans yet.</p>}</aside>
        <section className="panel"><div className="toolbar"><div><h3>{selectedScan?scanLabel(selectedScan).toUpperCase():'SCAN RESULTS'}</h3><span>{selectedScan?.status==='running'&&selectedScan.scannedBlocks?`${selectedScan.scannedBlocks.toLocaleString()} of ${selectedScan.totalBlocks.toLocaleString()} blocks scanned`:selectedScan?.error||'Newest contracts first'}</span></div><label className="search"><Search size={17}/><input value={filters.q??''} onChange={event=>setFilters(current=>({...current,q:event.target.value}))} placeholder="Address, name, symbol"/></label></div>
      ...
      <footer><p>Heuristic research tool — not financial advice and not a guarantee of contract safety.</p><p>Croikey gated · Ethereum ownership check · Individual EVM chain scans</p></footer></div>
