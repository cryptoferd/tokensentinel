...
export async function readDeploymentMetadata(address: `0x${string}`) {
  ...
  const val = <T>(i:number): T|null => common[i]?.status === 'fulfilled' ? common[i].value as T : null;
  const name = val<string>(0), symbol = val<string>(1), totalSupply = val<bigint>(2);
  if (isErc721) return { assetType:'ERC721' as const, name, symbol, decimals:null, totalSupply };
  const decimalsRaw = await client.readContract({ address, abi:erc20Abi, functionName:'decimals' }).catch(() => null);
  if (totalSupply == null || decimalsRaw == null) return null;
