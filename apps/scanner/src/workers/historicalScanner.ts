...
export async function runHistoricalScan(scanId:string,chainKey:string,lookbackMinutes:number) {
  ...
  try{
    ...
    for(let cursor=from;cursor<=tip;cursor+=BigInt(batchSize)){
      ...
      await Promise.all(numbers.map(blockNumber=>limit(async()=>{
        const block=await pacedRpc(chainKey,()=>client.getBlock({blockNumber,includeTransactions:true}));
        const creations=block.transactions.filter((tx:any)=>tx.to===null);
        await Promise.all(creations.map((tx:any)=>probeLimit(async()=>{
          try{
            const receipt=await pacedRpc(chainKey,()=>client.getTransactionReceipt({hash:tx.hash}));
          ...
          }catch(error){console.warn(`[history:${chainKey}] skipped creation ${tx.hash}:`,sanitizeRpcError(error));}
        })));
  ...
  }catch(error){const message=sanitizeRpcError(error);console.error(`[history:${chainKey}] ${scanId} failed:`,message);updateScanSession(scanId,{status:'failed',completedAt:Date.now(),error:message});}
